...
class APIModuleTest {
    ...
    fun `rich requests preserve multibyte content above plain limits for json and forms`() =
        withTestApi { settings, telegram, _ ->
            ...
            for ((type, body) in listOf(
                ContentType.Application.Json to json,
                ContentType.Application.FormUrlEncoded to listOf("text" to text).formUrlEncode()
            )) {
    ...
    @Test
    fun `rich format rejects unknown duplicate and invalid block shapes before delivery`() =
        withTestApi { _, telegram, _ ->
