...
internal class TelegramRichOutboxTest : MessagePollerFacadeTestSupport() {
    ...
    fun `text and voice AI replies persist rich plans before their first send`() = runBlocking {
        for (voiceInput in listOf(false, true)) {
            ...
            fixture.saveSettings(AppSettings(telegramToken = "100:token", ai = AISettings(agentEnabled = true, agentChatId = "123")))
            coEvery { fixture.telegram.getUpdatesForToken("100:token", 11, 30) } returns GetUpdatesResponse(ok = true, result = listOf(Update(11, message)))
            ...
            coEvery { fixture.telegram.getFileForToken("100:token", "voice-id") } returns FileResponse(true, TelegramFile("voice-id", "unique", filePath = "voice.ogg"))
            ...
            coEvery { fixture.telegram.sendRichMessageForToken("100:token", "123", any(), ReplyParameters(1)) } coAnswers {
    ...
    fun `quoted rich rejection falls back locally and resumes midway after restart`() = runBlocking {
        ...
        fixture.updates.completeAgentUpdate("100", 11, PendingTelegramReply(11, "123", source, ReplyParameters(1), deliveryPlan = plan))
        fixture.saveSettings(AppSettings(telegramToken = "100:token"))
        coEvery { fixture.telegram.getUpdatesForToken("100:token", 12, 30) } returns GetUpdatesResponse(ok = true)
        coEvery { fixture.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = plan[0].text), any()) } returns rejected
        ...
        try {
            ...
            coVerifyOrder {
                fixture.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = plan[0].text), ReplyParameters(1))
                fixture.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = plan[0].text), null)
        ...
        coEvery { restarted.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = "**后续**"), null) } returns accepted
        ...
        try {
            ...
            coVerifyOrder {
                ...
                restarted.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = "**后续**"), null)
            }
    ...
    fun `temporary rich failures keep format and quoted state until accepted`() = runBlocking {
        ...
        fixture.updates.completeAgentUpdate("100", 11, PendingTelegramReply(11, "123", "回复", ReplyParameters(1), deliveryPlan = plan))
        fixture.saveSettings(AppSettings(telegramToken = "100:token"))
        coEvery { fixture.telegram.getUpdatesForToken("100:token", 12, 30) } returns GetUpdatesResponse(ok = true)
        val attempts = AtomicInteger()
        val failures = listOf(HttpStatusCode.TooManyRequests, HttpStatusCode.ServiceUnavailable, HttpStatusCode.RequestTimeout)
        coEvery { fixture.telegram.sendRichMessageForToken("100:token", "123", InputRichMessage(markdown = "**回复**"), ReplyParameters(1)) } coAnswers {
            val pending = fixture.updates.getPendingTelegramReplies("100").single()
