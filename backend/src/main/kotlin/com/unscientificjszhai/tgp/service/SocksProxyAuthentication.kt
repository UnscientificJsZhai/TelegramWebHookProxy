...
private fun matchesProxyHost(configured: String, requested: String?): Boolean {
    ...
    if (!configured.isNumericAddress() || !requested.isNumericAddress()) return false
    return runCatching { InetAddress.getByName(configured) == InetAddress.getByName(requested) }.getOrDefault(false)
}
